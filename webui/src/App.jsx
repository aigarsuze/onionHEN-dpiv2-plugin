...
function App() {
  ...
  useEffect(() => {
    ...
    const apply = (status) => {
      ...
      if (Number.isInteger(status.api_port) && status.api_port > 0 && status.api_port <= 65535) {
  ...
  useEffect(() => {
    ...
    fetch('/api/config')
      ...
      .then((config) => {
        if (active && Number.isInteger(config.api_port) && config.api_port > 0 && config.api_port <= 65535) {
