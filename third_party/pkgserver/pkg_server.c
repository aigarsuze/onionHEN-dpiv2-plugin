 ...
 *
 * OnionHEN: compiled into the standalone DPI v2 plugin; pkg_server_main runs
 * on the plugin service worker thread.
 *
...
static void pkg_notify(const char *key, ...) {
    ...
    va_start(ap, key);
    vsnprintf(req.message, sizeof(req.message), dpi_i18n_translate(key), ap);
    va_end(ap);
    sceKernelSendNotificationRequest(0, &req, sizeof(req), 0);
}


#define LOG_FILE_PATH DPI_SERVER_LOG_PATH
...
static uint16_t g_server_port = ONION_PKGNET_PORT;
static uint16_t g_webui_port = ONION_WEBUI_PORT;
#define SERVER_PORT ((int)g_server_port)
#define WEBUI_PORT ((int)g_webui_port)
...
/* ------------------------------------------------------------------------- */
/* Web UI endpoints: single-file bundle + browser /api/ flow. */
/* ------------------------------------------------------------------------- */
...
static void webui_status_payload(char *body, size_t len) {
    ...
    snprintf(body, len,
             ...
             "\"phase\":\"%s\",\"error\":\"%s\",\"busy\":%s,"
             "\"api_port\":%u,\"name\":\"%s\","
             "\"total\":%llu,\"cid\":\"%s\"}",
             ip, g_webui_staged ? "true" : "false", lang, phase, err,
             busy ? "true" : "false", (unsigned)g_server_port, esc_name,
...
static void *webui_conn_handler(void *arg) {
    ...
    if (strcmp(req.path, "/") == 0 ||
        ...
        webui_send_index(fd);
    } else if (strcasecmp(req.path, "/api/config") == 0) {
        char body[96];
        snprintf(body, sizeof(body),
                 "{\"ok\":true,\"api_port\":%u,\"webui_port\":%u}",
                 (unsigned)g_server_port, (unsigned)g_webui_port);
        send_json(fd, 200, "OK", body);
...
static void takeover_previous_instance(void) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (port_is_listening()) {
            ...
            log_line("takeover: :%d owned by a non-pkg-server process; "
                     "leaving it alone", SERVER_PORT);
            return;
        }
    ...
    log_line("takeover: pkg-server detected on :%d - replacing it",
             SERVER_PORT);
...
/* Service start: reset the one-shot shutdown/backoff state so the worker can
...
void pkg_server_set_ports(uint16_t api_port, uint16_t webui_port) {
    if (api_port == 0 || webui_port == 0 || api_port == webui_port ||
        g_srvfd >= 0 || g_webui_fd >= 0) {
        return;
    }
    g_server_port = api_port;
    g_webui_port = webui_port;
}

/* Service stop: mirror the /shutdown handler (flag + wake the accept loops). */
...
/* Service readiness: both listeners are bound and the worker is not stopping. */
int pkg_server_is_listening(void) {
    return g_srvfd >= 0 && g_webui_fd >= 0 && !g_shutdown;
}
