# OnionHEN DPI v2 Plugin API

Two configurable servers run in the plugin process
(`third_party/pkgserver/pkg_server.c`):

| Port | What | Kind |
|------|------|------|
| **9090** (default) | pkg install / chunk upload API | JSON (transfer, preflight, control) |
| **12800** (default) | web UI (single-file bundle) + status/progress | HTTP + SSE |
...
## 1) DPI web UI — TCP 12800 by default
...
| `GET /api/config` | returns the current API and WebUI listener ports |
...
```json
data: {"ok":true,"ip":"192.168.1.10","staged":true,"language":"es","phase":"uploading","error":"","busy":false,"api_port":9090,"name":"MyGame.pkg","total":4194304000,"cid":"UP0001-XXXX-XXXXX_00-XXXXXXXXXXXXXXXXXX","bytes":123456789}
...
| `language` | string | Web UI locale as a BCP-47 tag: `zh-Hans`, `zh-Hant`, `en`, `ja`, `ko`, `fr`, `de`, `it`, `es`, `pt-BR`, `pl`, `ru`, `ar` or `th`. Resolved from the console system language and refreshed while the plugin runs; `en` is the fallback |
| `api_port` | number | Current DPI transfer API port configured by the plugin XML page |
...
## 2) Install / chunk API — TCP 9090 by default

Default base for requests: `http://<console>:9090`. The WebUI obtains the
current port from `GET /api/config` on its same-origin listener.
...
`staged:"kept"` means the pkg is left on disk for a retry. The plugin emits
localized notifications for receiving / accepted / failed states.
...
and the plugin service stop path; keep it.
...
- The web UI bundle (`webui/dist/index.html`) is embedded into the
  plugin during the build (`CMakeLists.txt`); after editing
  `webui/src`, rebuild it with `npm run build` in `webui`.
- `npm run dev` in `webui` serves on `:5173` and proxies `/api` to
  `http://127.0.0.1:8081` (local dev target).
- By default the bundle is served same-origin on `:12800`, so the SSE URL
  is a relative `/api/stream`; the chunk API base is built from
  API listener returned by `/api/config`.
